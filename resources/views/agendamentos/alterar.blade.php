...
@section('content')
    ...
    <div class="m-portlet">
		...
		<form id="createform" class="m-form m-form--fit m-form--label-align-right ajax-form" method="post" action="{{route('agendamento.upd')}}">
            <input type="hidden" name="_token" id="token" value="{{ csrf_token() }}">
            <input type="hidden" name="id_agendamento" id="id_agendamento" value="{{ $agendamento->id }}">
				...
				<div class="row" style="padding-left:1em; padding-right:1em;">
					<div class="col-xs-12 col-md-6 col-sm-6 col-lg-6" style="margin-bottom:1em;">
						...
						<select class="form-control m-input" id="id_usuario" name="id_usuario" style="width:100%">
							...
							{
								?>
								<option value="<?php echo $usuario_val['id']; ?>" <?php echo ($usuario_val['id'] == $agendamento->id_usuario) ? "selected" : ""; ?>><?php echo $usuario_val['nome']; ?></option>
								<?php
							}
							?>
                        </select>
					...
					<div class="col-xs-12 col-md-6 col-sm-6 col-lg-6" style="margin-bottom:1em;">
						...
						<select class="form-control m-input" id="servicos" name="servicos[]" style="width:100%" multiple="multiple">
							<?php 
							$servicos_ag_arr = explode(",",$servicos_ag[0]->ids_servicos);
							foreach($servicos as $servico_val)
							{
								?>
								<option value="<?php echo $servico_val['id']; ?>" <?php echo (in_array($servico_val['id'], $servicos_ag_arr)) ? "selected" : ""; ?>><?php echo $servico_val['servico']; ?></option>
								<?php
							}
							?>
                        </select>
					...
					<div class="col-xs-12 col-md-6 col-sm-6 col-lg-6">
						...
						<?php 
						$hora_agendamento_exp = explode(":", $agendamento->hora_agendamento);
						$hora_agendamento_val = $hora_agendamento_exp[0];
						?>
						<input type="number" id="hora_agendamento" name="hora_agendamento" class="form-control m-input" required="" value="{{ $hora_agendamento_val }}" min="8" max="17" aria-invalid="false">
					</div>
					<div class="col-xs-12 col-md-6 col-sm-6 col-lg-6">
						<label for="status">Status</label>
						<select class="form-control m-input" id="status" name="status" style="width:100%">
...
<script>
	$(document).ready(function() {
		...
		var dateAg = new Date(<?php echo "'$agendamento->data_agendamento'"; ?>);
		...
		$("#data_agendamento").datetimepicker().datetimepicker("setDate", dateAg);
